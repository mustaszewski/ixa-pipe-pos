...
public abstract class AbstractCRFTaggerTrainer implements TaggerTrainer {
   ...
   *           the io exceptions
   */
  public AbstractCRFTaggerTrainer(final TrainingParameters params) throws IOException {
  ...
  public final POSModel train(final TrainingParameters params) {
    ...
    // training model
    SequenceClassificationModel<String> trainedModel = null;
    POSModel seqModel = null;
    CRFTrainer crfTrainer = new CRFTrainer();
    POSEvaluator posEvaluator = null;
    Map<String, String> manifestInfoEntries = new HashMap<String, String>();
    try {
      POSContextGenerator contextGenerator = posTaggerFactory.getPOSContextGenerator();
      POSSampleSequenceStream ss = new POSSampleSequenceStream(this.trainSamples, contextGenerator);
      //trainedModel = POSTaggerME.train(this.lang, this.trainSamples, params, getPosTaggerFactory());
      trainedModel = crfTrainer.doTrain(ss);
      seqModel = new POSModel(this.lang, trainedModel, manifestInfoEntries, getPosTaggerFactory());
      final POSTaggerME posTagger = new POSTaggerME(seqModel);
