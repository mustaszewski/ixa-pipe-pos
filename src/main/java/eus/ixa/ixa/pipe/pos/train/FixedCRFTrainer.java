 ...
 * @version 2014-11-26
 */
public class FixedCRFTrainer extends AbstractCRFTaggerTrainer {
   ...
   *           the io exception
   */
  public FixedCRFTrainer(final TrainingParameters params) throws IOException {
    super(params);
  ...
  private final POSTaggerFactory getTrainerFactory(
    ...
    /*
    else if (featureSet.equalsIgnoreCase("CRFFeatures")){
        try {
          posTaggerFactory = POSTaggerFactory.create(
              CRFFactory.class.getName(), ngramDictionary, null);
          System.out.println("!!!!!!!! CRF Feature Factory choosen!!!!!!!!"); //Debug Only
        } catch (final InvalidFormatException e) {
          e.printStackTrace();
        }
      }
